...
def ts_selection_routine(model, X_init, Y_init, horizon, objective_func, x_bound, **kargv):
    """
    args:
        model: GPyOpt.models.quadrature.GaussianFiniteQuadrature
    """
    print('=== Start TS selection routine ===')
    m = kargv['m']
...
def ts_suggest(model, epsilon, x_bound, m = 1000, kernel_type='RBF'):
    xt, report_value = ts_suggest_x(model, epsilon, x_bound, m = m, kernel_type=kernel_type)
...
def ts_suggest_x(model, epsilon, x_bound, m = 1000, kernel_type='RBF'):
    """
    args:
        model: GPyOpt.models.quadrature.GaussianFiniteQuadrature
    """
    ...
    def obj_func(x):
        ...
        ts_val = model.w_quadrate.integrate(ts_vals, axis=0)
        return -ts_val
